...
runs = [
    ...
    ("looser_pullback", {"TREND_PULLBACK_ATR_TOLERANCE": "0.80"}),
    ("faster_protection", {"PARTIAL_PROFIT_RR": "0.75", "BREAK_EVEN_RR": "0.75", "TRAILING_ATR_MULT": "1.8"}),
    ("slower_protection", {"PARTIAL_PROFIT_RR": "1.25", "BREAK_EVEN_RR": "1.25", "TRAILING_ATR_MULT": "2.8"}),
    ("lower_spread_assumption", {"GOLD_BACKTEST_SIMULATED_SPREAD": "0.15"}),
    ("higher_spread_assumption", {"GOLD_BACKTEST_SIMULATED_SPREAD": "0.40"}),
    ("strict_combo", {"TREND_PULLBACK_ATR_TOLERANCE": "0.45", "PARTIAL_PROFIT_RR": "0.75", "BREAK_EVEN_RR": "0.75", "TRAILING_ATR_MULT": "1.8"}),
]

results = []
for name, overrides in runs:
    out_dir = sweeps_root / name
    if out_dir.exists():
        shutil.rmtree(out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    cache_dir = sweeps_root / f"_cache_{name}"
    if cache_dir.exists():
    ...
    row = {"run_name": name, "success": success}
